...
public class PlayerMotion : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(velocity<0f&&controller.isGrounded)
        {
            velocity= -1f;
        }
        else
        {
            velocity = -9.81f * 3f * Time.deltaTime;
        }
        Vector3 motion = new Vector3(dx, velocity, dz);
    ...
    {
        if (other.tag == "Sword" && isAttacked && !other.gameObject.GetComponent<PaladinSwordAttack>().enemy.isDead)
