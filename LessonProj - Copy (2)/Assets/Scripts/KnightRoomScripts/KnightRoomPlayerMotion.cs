...
public class KnightRoomPlayerMotion : MonoBehaviour
{
    ...
    private KnightRoomPaladinMotion enemy;
    ...
    IEnumerator Attack()
    {
        ...
        if (Physics.Raycast(aCamera.transform.position, aCamera.transform.forward, out hit, 6f))
        {
            for (int i = 0; i < allEnemys.Length; i++)
            {
                ...
                {
                    enemy = allEnemys[i].GetComponent<KnightRoomPaladinMotion>();
    ...
    {
        if (other.tag == "Sword" && isAttacked && !other.gameObject.GetComponent<KnightRoomPaladinSwordAttack1>().enemy.isDead)
