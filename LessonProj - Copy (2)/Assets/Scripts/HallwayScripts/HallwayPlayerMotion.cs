...
public class HallwayPlayerMotion : MonoBehaviour
{
    ...
    private HallwayPaladinMotion enemy;
    ...
    IEnumerator Attack()
    {
        ...
        if (Physics.Raycast(aCamera.transform.position, aCamera.transform.forward, out hit, 6f))
        {
            for (int i = 0; i < allEnemys.Length; i++)
            {
                ...
                {
                    enemy = allEnemys[i].GetComponent<HallwayPaladinMotion>();
    ...
    {
        if (other.tag == "Sword" && isAttacked && !other.gameObject.GetComponent<HallwayPaladinSwordAttack>().enemy.isDead)
